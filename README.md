...
- **국장+미장 통합 실적 캘린더** — 상단 마퀴 바로 아래 한 줄에 "오늘 N건 · 7일 M건"
  ...
  표시합니다. 종목별 EPS 추정/실제, 서프라이즈 %, 시총, 발표 시간(BMO 장 시작 전 / AMC
  장 마감 후 / TAS 장중 / TNS 시간 미정) 배지가 함께 보이며 미장(🇺🇸) / 국장(🇰🇷) / 기타(🌏)
  필터, 심볼·회사명 검색을 지원합니다. 데이터는 야후 파이낸스의 visualization calendar
  API에서 직접 받으며, KV에 6시간 단위로 캐시되어 사용자가 늘어도 추가 비용이 없습니다
