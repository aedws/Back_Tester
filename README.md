...
- **상단 마켓 마퀴 컴팩트화** — 11개 지표를 데스크톱 한 줄에 펼쳐서 스크롤 없이 한눈에. 30초
  ...
  타임스탬프 표시
- **국장+미장 통합 실적 캘린더** — 상단 마퀴 바로 아래 한 줄에 "오늘 N건 · 7일 M건"
  바를 항상 노출. 클릭하면 모달이 열리고 다음 7일간의 실적 발표를 **날짜별로 그룹화**해서
  표시합니다. 종목별 EPS 추정, 시총, 발표 시간(BMO 장 시작 전 / AMC 장 마감 후 / TAS 장중
  / TNS 시간 미정) 배지가 함께 보이며 미장(🇺🇸) / 국장(🇰🇷) / 기타(🌏) 필터, 심볼·회사명
  검색을 지원합니다.
  - **미장**: [Nasdaq 공식 캘린더 API](https://api.nasdaq.com/api/calendar/earnings)에서
    날짜별로 가져오므로 매우 안정적이고 600~800건/주 수준으로 풍성합니다.
  - **국장**: 핵심 3개 종목(삼성전자 005930.KS · SK하이닉스 000660.KS · 현대차
    005380.KS)에 대해 야후 `quoteSummary({ calendarEvents })`를 폴링. 야후 인증
    이슈로 일시 차단되는 경우가 있어 모달 푸터의 "데이터 소스" 줄에서 상태를 즉시
    확인할 수 있습니다.
  - **회사 로고**: 미장은 financialmodelingprep 이미지 CDN, 국장 3개는 Clearbit
    Logo API에서 도메인 매핑으로 가져와 종목 행에 함께 표시. 로딩 실패 시 심볼
    이니셜이 색깔 박스로 자동 fallback 됩니다.
  - **KV 7일 캐시**: 한 번 받으면 일주일 그대로 사용. Vercel 함수 콜드 스타트나 야후
    차단에도 데이터가 살아 있습니다. 결과가 0건이면 15분 짧은 캐시만 걸어 빠르게 회복
- **모든 차트에서 휠/핀치/드래그 줌** — 비교/자산곡선/가격/재투자 비교/상세(가격·볼륨·RSI·Stochastic)
  차트 전부에서:
  - **PC**: 차트 위에 커서를 올리고 **마우스 휠**로 줌 인/아웃 (커서 위치 중심으로 확대·축소).
    페이지 스크롤은 차트 영역에서만 잠시 막힙니다.
  - **모바일 / 트랙패드**: 두 손가락 **핀치**로 줌 (손가락 중간점 중심).
  - **드래그 박스 줌**: 가로로 드래그한 만큼만 잘라서 보고 싶을 때.
  - **더블클릭** 또는 우상단 "↺ 리셋" 버튼으로 원복.
  상세 차트의 4개 패널은 줌 상태를 공유해서 가격·지표가 항상 같은 X축 구간을 보여주며,
  Y축은 줌 윈도우에 맞춰 자동 재조정되어 작은 변동도 크게 볼 수 있습니다
